...
export function bodyMaterial(spec, avatar, lod) {
  const suffix = lod ? '_lod' : '';
  const material = new THREE.MeshStandardMaterial({ color: 0xffffff, roughness: .74, metalness: 0, map: texture('body_albedo' + suffix, opts), normalMap: lod ? null : texture('body_normal', { ...opts, srgb: false }) });
  if (material.normalMap) material.normalScale.set(.85, .85);
  const u = {
    uMask1: { value: texture('body_mask1' + suffix, { ...opts, srgb: false }) }, uMask2: { value: texture('body_mask2' + suffix, { ...opts, srgb: false }) },
  ...
  material.onBeforeCompile = s => {
    ...
    s.fragmentShader = 'uniform sampler2D uMask1, uMask2; uniform vec3 uPal[7]; uniform float uMean[7]; uniform float uDetail[7]; uniform vec3 uBeard, uHair;\n' + s.fragmentShader.replace('#include <map_fragment>', `float chainsJersey = 0.;
      ...
      { vec4 m1 = texture2D(uMask1, vMapUv), m2 = texture2D(uMask2, vMapUv);
        ...
        vec3 col = base;
        for (int i = 0; i < 7; i++) { float d = mix(1., clamp(lum / uMean[i], .25, 1.8), uDetail[i]); col = mix(col, uPal[i] * d, w[i]); }
        float bz = m2.a; float zone = bz > .62 ? uBeard.z : bz > .37 ? uBeard.y : bz > .12 ? uBeard.x : 0.;
