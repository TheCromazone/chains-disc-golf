...
## Round four — canopy volume only

- Committed the inherited `?fps=1` overlay first, unchanged, as `c15b0ee`. It reports the running quality, smoothed frame rate, render pixel ratio and viewport. The three untracked round-prompt documents remain user-owned and are excluded from implementation commits.
- Replaced runtime crown primitives with six Blender-authored shapes: three variants each for pine and deciduous. Each uses three closed irregular shells. Blender casts 64 surface-hemisphere rays and 64 sky-hemisphere rays per vertex, baking overlap occlusion into grayscale vertex color. Exposed vertices keep the existing palette; sheltered joins darken within the same toon material.
- Embed quantized positions and baked occlusion in a small runtime module. Lite and empty-manifest play need no model loader or additional texture. Tree placement, collision radii, wind bend, 64 m spatial partition, terrain and painted tiles remain unchanged. Variant selection uses spatial noise without consuming the placement RNG.
- The independent first review still selected the reference: joins were too shallow and facets too prominent. The second build raises unequal shoulders into the main crown and strengthens only occluded vertex shading. It stays within the previous Lite crown triangle caps. Review evidence is kept under `docs/qa/r4/`.
- Device measurements must come from the user's physical Android/iPhone overlay readings on Pine Hollow hole one. Until supplied, all four measured values remain null. Lite below 55 FPS or Full below 40 FPS on either device triggers work on that device's resolution/foliage bottleneck. Desktop counters, emulator timings and triangle counts cannot satisfy this gate.
- Rejected the third review's provisional outside-canopy finding after checking the closer portrait camera: wide framing alone had concealed the issue. The fourth build uses one continuous visible outer shell, two tucked interior shells, and Blender-authored normals transferred from a whole-crown envelope. A per-instance diagnostic then showed that the critic's alleged attached right-hand lobe was a separate neighboring tree; the critic explicitly corrected that attribution and closed the spherical-assembly finding.
...
### Round five, second pass — textures off the toon ramp (2026-09-15)
- The user asked for surfaces "less Wii Sports, more like the frisbee game but better" and a natural disc grip. Toon materials are gone everywhere: `toonMaterial()` now returns a `MeshStandardMaterial` so every prop upgraded at once, the toon ramp module was deleted, and the athlete's slots map to roughness plus a tiled fabric weave (`jersey_pattern`) and skin pore normal map through the UV set the Blender builder now emits.
- Ported from the frisbee game's measured solve: photo grass tile washed 58% toward white over vertex-colour palette (`washedTexture`), blade-scale normal at 8x the tile repeat, macro value noise against tiling, one sun at 2.5 with PCF `shadow.radius` and bias derived from texel size, hemisphere fill, fog matching the sky horizon, ACES 1.05 on every tier, Fresnel rim added as a light and damped by luma, and a grade pass (lift, saturation, grain, vignette) with bloom at .22/.4/.93 instead of SSAO.
- Disc grip: the disc now lives in the hand's own frame. Idle carries it by the rim beside the thigh; during a throw its orientation is derived from the clip's release pose (`releaseFrame`) so it rides the wrist and is level with the planned release normal at phase .62. The relaxed idle arm (shR .28, elR .5) required re-authoring all 32 clips.

## Round six - the frisbee project's athlete, aim line, wind streamlines (2026-09-15)

- The user rejected the lofted athlete twice ("i still don't like the character models, look at what they look like in my ultimate frisbee game"). That game's body is a Meshy 7 image-to-3D scan the user already owns, so `tools/build-golfer-v3.py` reuses it instead of authoring another figure: rotate to face -Z, straighten the A-pose with pose-bone aims, apply, merge its 24 joints into the eleven ChainsRig bones, merge seam-split vertices, decimate to 12.4k / 6.2k, bake normals and occlusion from the high-res copy. The source GLB stays in the frisbee project (12 MB); the builder takes `--source`.
- Customisation on a scanned body: a world-position bake plus colour rules classify the albedo into skin, shirt, shorts, hair, socks, shoes, iris and beard zones; `src/body-material.js` recolours each region as palette x (baked luminance / region mean). Eyes, brows, nose and mouth options were removed from the locker (the face is real now); eye colour and facial hair moved into the masks; glasses stay a decal on the measured eye line.
- Hair and headwear are grown from the skull (head copy, decimate, smooth, push out, hairline cut, solidify, drop inward faces). Shrinkwrapped spheres folded on themselves; distance-based inner-face removal ate the shells.
- Textures ship as WebP (albedo, normal) and posterized PNG masks resized per channel, because Pillow resamples RGBA through premultiplied alpha and erased the masks.
- The flight preview became a `Line2` ribbon (3 px white dashes, 6 px dark outline, flowing dash offset) with a ground chevron at the lie; the one-pixel dashed line had vanished against the brighter physically based grass.
- Wind streaks became tapered camera-facing ribbons with a spatially sampled tail (about 7 m) advected by wind plus a curl, fading toward the tail and respawning upwind of the focus. The first cut respawned every frame because the spawn radius exceeded the far threshold.
- Codex was not delegated to: the pass is Blender CLI, Python and three.js, and its budget is low. Unreal and Unity were considered and skipped: a three.js target gains nothing from them. The requested skill repos (Claude-Code-Game-Studios, godogen, claude-unity-game-studio, claude-code-game-development, awesome-claude-code-subagents) were cloned and read; the CCGS test-evidence template shapes `docs/qa/r6/README.md`; godogen's asset-gen needs paid Tripo3D/Gemini keys and was not used.

## Round seven - female body, grip, seam-free bake, glasses, trees (2026-09-16)

- Wristband rings floated around the hands because the wrist was measured from every vertex at wrist height on that side, thigh included. Measured from the hand's neighbourhood now, clamped.
- The albedo had shipped with linear values in an sRGB file (Blender writes float buffers untransformed), which is why every skin tone rendered dark and muddy; the builder encodes sRGB itself before saving.
- Decimating Meshy's atlas smeared UVs across island gutters (white and black specks, "weird lines" on the face). Each body is unwrapped afresh (smart project + pack), baked with a 16 px margin, and the region classifier votes over a neighbourhood, with a baked forearm map so skin below the elbow can never be shirt.
- Blender's decimate vertex-group weighting is nonlinear; two passes with pinned regions (weight 0 never collapses) give an explicit face-and-hands budget on any scan.
- Fingers are bent at three hinges before the bake so the hand hooks the disc rim; the socket offset stays where the carry and release math expect it.
- The female body is a Meshy 7 image-to-3D from a generated studio concept, made through the user's Higgsfield credits (44 for the model, about 4 for images). Same builder, `--variant f`; the source GLB lives in `art/meshy/` outside git. She shares the rig and clip set; `legScale` in the rig extras scales the clips' hip drops.
- Glasses moved from a decal to geometry; the square rim had been scaled about the world origin and sat on the chest. The beard channel now carries zone id plus softness so facial hair fades.
- Course trees for the Disc Golf Masters look: Blender branch tubes with crossed photo leaf cards, transparent texels dilated so mipmaps stay green; Full only, instanced with cut-out shadows. Lite keeps the embedded crowns.
- The reference video was consumed through the claude-video watch skill (captions) plus frames viewed in the browser pane; yt-dlp could not download the stream without a JS runtime, so no frames were extracted locally.
- Selected-to-active bakes with a 2 cm cage started rays inside whichever part the arm or thigh was touching, so forearms baked shirt and the belly baked skin ("shirt lines through the skin"). The game body sits within millimetres of the scan; cage .007 / ray .022 keeps every ray on its own part.
- Skin is no longer painted over with a palette colour (that read flat and red): the shader shifts the scan's own skin by the ratio of the chosen tone to the bake's mean skin colour (`skinMean` in the rig extras), with a 30 % pull toward the tone, so cheeks, knuckles and veins survive every skin option.
- The hands hang inside the shorts' height band, so a shadowed wrist texel voted dark and became shorts (black streaks, or white ones once the cloth gain lifted them). Bone-weight maps (forearm, upper arm below the sleeve) now veto both shirt and shorts on the arms; a per-face check in Blender (forearm faces sampling the mask at their UV centroid) is the regression test for this class of bug.
- The decisive fix for the arms: decimate, unwrap and bake in the scan's own A-pose (arms clear of the torso) and only then straighten into the ChainsRig rest. Straightened arms hang against the hips, so no cage size kept forearm rays out of the shirt. Forearm faces sampling the wrong region went from 117 to 8 degenerate slivers out of 1,619.
- AO rays that find no scan surface come back black; those texels are treated as open instead of shading the shirt with specks.
- Albedo transfer rays that start under a fold the decimation cut away miss the scan and leave black texels (grey specks on a white shirt). Those texels are inpainted from their neighbours after the bake; the miss fraction is logged in the build report.
